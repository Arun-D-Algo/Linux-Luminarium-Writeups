...
### Solve
**Flag:** `pwn.college{se2UVrxITLx8KxAQpD8vPo-nuKu.QX4MDO0wiNyUDN0EzW}`

This challenge focused on identifying and interacting with running processes instead of relying on file listings. Since the /challenge directory could not be listed and the executable had been renamed to a random filename, the only way to find it was by inspecting active processes using ps. By running ps -efww (to avoid truncation) and filtering for “challenge,” the full path of the renamed executable was revealed. Once identified, the binary was executed directly to retrieve the flag. The task emphasized that processes already running in memory can expose information even when filesystem access is restricted.

```bash
hacker@processes~listing-processes:~$ ps -efww | grep challenge
root         134       1  0 11:35 ?        00:00:00 /challenge/30372-run-4976
hacker       363     352  0 11:39 pts/0    00:00:00 grep --color=auto challenge
hacker@processes~listing-processes:~$ /challenge/30372-run-4976
Yahaha, you found me! Here is your flag:
pwn.college{se2UVrxITLx8KxAQpD8vPo-nuKu.QX4MDO0wiNyUDN0EzW}
Now I will sleep for a while (so that you could find me with 'ps').
```

### New Learnings
Learned how ps -ef and ps aux provide different but overlapping views of system processes, and how the ww option prevents command path truncation. Reinforced understanding of process IDs (PID), parent process IDs (PPID), and how running programs can be discovered even when directory listing is blocked. Also strengthened the concept that Linux systems expose runtime state through process inspection, making process enumeration a powerful diagnostic and investigative tool.